...
void H5Synapses::singleConnect(NESTSynapseRef synapse, nest::Node* const target_node, const nest::thread target_thread, DictionaryDatum& d ,std::vector<const Token*> v_ptr, uint64_t& n_conSynapses)
{
    ...
    if (nest::kernel().node_manager.is_local_node(target_node)) {
        ...
        std::vector<double> values(synapse.params_.first(), synapse.params_.end());
    ...
    #pragma omp parallel default(shared) reduction(+:n_conSynapses_sum)
    {
        ...
        //only connect neurons which are on local thread otherwise skip
        {
            ...
            for (int i=0;i<synapses_.size();i++) {
                ...
                // synapse belongs to local thread, connect function is thread safe for this condition
                if (target_thread == thrd) {
                    stride_c++;
                    //only connect each stride ones
                    if (stride_c==1)
                        singleConnect(synapses_[i], target_node, target_thread, d, v_ptr, n_conSynapses_tmp);
                    if (stride_c>=stride_)
                        stride_c = 0;
                }
            }
...
CommunicateSynapses_Status H5Synapses::CommunicateSynapses()
{
    ...
    MPI_Alltoallv(send_buffer.begin(), sendcounts, sdispls, MPI_UNSIGNED, recvbuf.begin(), recvcounts, rdispls, MPI_UNSIGNED, MPI_COMM_WORLD);
...
void H5Synapses::integrateMapping()
{
    #ifdef SCOREP_COMPILE
...
void H5Synapses::sort()
{
    ...
    if (synapses_.size()>1) {
        ...
        std::sort(rank_idx.begin() rank_idx.end(), bool cp(const intpair&, const intpair& r) {return l.first<r.first;});
        for (int i=0; i<rank_idx.size(); i++) {
            rank_idx[i].first = i;
        }
        
        //sort again for fast forward swapping of elements
        std::sort(rank_idx.begin() rank_idx.end(), bool cp(const intpair&, const intpair& r) {return l.second<r.second;});
        
        //create buf object
        unsigned int source_neuron_tmp;
        unsigned int node_id_tmp;
        std::vector<char> pool_tmp(synapses_[ix].params_.size()*sizeof(float) + sizeof(int));
        NESTSynapseRef buf(source_neuron_tmp, node_id_tmp, synapses_[ix].params_.size(), pool_tmp.begin());
...
void H5Synapses::import(DictionaryDatum& dout)
{
    ...
    H5SynapsesLoader synloader(filename_, synapses_.prop_names,n_readSynapses,n_SynapsesInDatasets, num_syanpses_per_process_, last_total_synapse_);
