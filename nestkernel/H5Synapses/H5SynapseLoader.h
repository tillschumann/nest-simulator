...
public:
    ...
    , fixed_num_syns_( fixed_num_syns )
  {
    ...
    if ( lastSyn > 0 )
    {
      ...
      H5Tinsert( syn_memtype_,
        "target",
        HOFFSET( NESTNodeSynapse, property_pool_ ),
        H5T_NATIVE_INT ); // first entry in pool is target id
      ...
      for ( int i = 0; i < prop_names.size(); i++ )
        H5Tinsert( syn_memtype_,
          ...
          HOFFSET( NESTNodeSynapse, property_pool_ )
            + ( 1 + i ) * sizeof( float ),
          H5T_NATIVE_FLOAT ); // following entries in pool are param values
