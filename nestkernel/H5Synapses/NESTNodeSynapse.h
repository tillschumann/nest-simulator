...
struct NESTSynapseRef
{
  ...
  class ParamPtr
  {
    float* ptr_;
    size_t n_;
    ParamIterator( float* ptr, const size_t& n )
      : ptr_( ptr )
    {
    }
  ...
  NESTSynapseRef( int& source_neurons,
    int& node_id,
  ...
  void
  NESTNodeSynapse::deserialize( std::vector< int >& buf )
  {
  ...
  void
  NESTNodeSynapse::integrateMapping( const GIDCollection& gidc )
  {
    source_neuron_ = gidc[ source_neuron_ ];
    target_neuron_ = gidc[ target_neuron_ ];

    const nest::index vp =
      nest::kernel().vp_manager.suggest_vp( target_neuron_ );
  ...
  swap( NESTSynapseRef& r )
  {
    ...
    NESTSynapseRef buf(
      source_neuron_tmp, node_id_tmp, params_.size(), pool_tmp.begin() );
